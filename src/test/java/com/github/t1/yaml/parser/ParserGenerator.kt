...
class ParserGenerator(private val spec: Spec) {
    ...
    private fun generateJavaCode() {
        var className = JAVA_SOURCE.fileName.toString()
        assert(className.endsWith(".java"))
        className = className.substring(0, className.length - 5)
        try {
            Files.newBufferedWriter(JAVA_SOURCE).use { writer -> generateJavaCode(className, writer) }
        } catch (e: IOException) {
            throw RuntimeException("while generating $JAVA_SOURCE", e)
        }
    ...
    fun generateJavaCode(className: String, out: Writer) {
        JavaCodeGenerator(className, out).write(spec.productions)
    }
